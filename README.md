...
Governance multisigs are sourced from the Sushi operations spreadsheet. Ethereum, Base, Polygon,
Arbitrum, and BNB Chain are populated. The sheet does not currently list Unichain, World Chain, or
Robinhood Chain, so those three remain deliberately unresolved. Fixed wrapped-native token addresses
...
1. Confirmation of the pool owner and protocol-fee-controller owner for Unichain, World Chain, and Robinhood Chain.
